...
public enum AppSimulationDecision { WouldAllow, WouldBlock, BlockUnknown }
...
public sealed record ObservedApp(string ProfileId, int SessionId, AppIdentity Identity, DateTimeOffset FirstSeenUtc, DateTimeOffset LastSeenUtc);
...
{
    public static AppPolicyEvaluation Evaluate(AppIdentity identity, IEnumerable<AppRule> rules, DefaultAppPolicy defaultPolicy)
    {
